...
export default function DigitalMedia() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const currentPrice = selectedProduct ? getProductPrice(selectedProduct, accountType) : 0;
      const res = await validateCouponMutation.mutateAsync({
        code: couponCode,
        productId: selectedProduct?.id,
        productType: 'digital',
        category: selectedProduct?.type,
        isPreVenda: !!selectedProduct?.isPreVenda,
        isEconomia: !!selectedProduct?.showInEconomia,
  ...
  const filteredProducts = products.filter((p: any) => {
    ...
    if (selectedGenre) {
      if (selectedGenre === "Pré Venda" || selectedGenre === "Pré-Venda" || selectedGenre === "Pre Venda") {
        ...
        matchesGenre = matchesCategoryProp || matchesKeyword;
      }
    }
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2.5 sm:gap-4">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="card-neon">
                <div className="skeleton-shimmer h-36 sm:h-52 rounded-t-xl" />
                <div className="p-2.5 sm:p-3">
                  <div className="h-3 sm:h-4 skeleton-shimmer rounded mb-2" />
                  <div className="h-2.5 sm:h-3 skeleton-shimmer rounded w-2/3" />
                </div>
              </div>
            ))}
          </div>
