...
async function seed() {
  console.log("Iniciando a inserção e atualização dos jogos com imagens...");
  ...
  for (const game of games) {
    ...
    if (existing.length > 0) {
      console.log(`[-] Jogo já cadastrado, atualizando imagem: ${game}`);
      await db
  ...
  console.log(`\nProcesso concluído! Inseridos: ${insertedCount}, Atualizados com imagem: ${updatedCount}`);
  process.exit(0);
}
